Public Class Frm_CamionesAndenRecepcion
    ...
    Private Sub Frm_CamionesAnden_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        '                                             "FROM clientes, choferes, zchecklist WHERE cl_rutcli=cli_rut " +
        '                                             "AND cl_chorut=cho_rut AND cl_estpor<>'0' AND cl_estfrigo='0' " +
        '                                             "AND cl_mov<>'1' AND (cl_des is null OR convert(date,Cl_Lleg,103)>=convert(date,DATEADD(day,-1,GETDATE()),103) AND Cl_Estfrigo=0)")

        DgvCamiones.DataSource = fnc.ListarTablasSQL("select a.cl_fol,a.cl_rutcli,b.cli_nomb,a.cl_chorut,c.cho_nombre,a.Cl_Mov from zCheckList a with(nolock) inner join clientes b with(nolock) on(a.Cl_RutCli=b.cli_rut) inner join choferes c with(nolock) on(a.Cl_chorut=c.cho_rut) left outer join Camiones_Carga_Descarga d with(nolock) on(a.Cl_fol=d.Folio_Camion) where a.Cl_EstPor<>'0' and a.Cl_Estfrigo='0' and a.Cl_Mov<>'1' and (cl_des is null OR (convert(date,Cl_Lleg,103)>=convert(date,DATEADD(day,-1,GETDATE()),103) AND Cl_Estfrigo=0)) and ISNULL(d.Estado_Recepcion,0)=0")
    ...
    Private Sub DgvCamiones_CellContentClick(sender As System.Object, e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DgvCamiones.CellContentClick
