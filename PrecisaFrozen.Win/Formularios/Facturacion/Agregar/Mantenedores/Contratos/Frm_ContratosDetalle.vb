Public Class Frm_ContratosDetalle

    Dim fnc As New Funciones

    Public contrato As String = ""
    Public contratonombre As String = ""
    ...
    Private Sub Frm_ContratosDetalle_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Dim tablaservicios As DataTable = fnc.ListarTablasSQL("SELECT Dcon_activo, Dcon_ServCod, Serv_Nom, CONVERT(varchar(7), Dcon_Valor) AS Dcon_Valor, Dcon_Moneda, Dcon_Medida, Serv_Esc " +
                                                              "FROM ContratosDetalle, FacServicios  WHERE Dcon_ServCod=Serv_Cod AND Dcon_Codigo='" + contrato + "'")
    ...
    Private Sub dgv_servicios_CellContentClick(ByVal sender As Object, ByVal e As DataGridViewCellEventArgs) Handles dgv_servicios.CellClick
        ...
        ElseIf dgv_servicios.Columns(e.ColumnIndex).Name = "Detalle" Then
            ...
            frm.txtcontratocod.Text = contrato.ToString
            frm.txtcontratonom.Text = contratonombre.ToString()
    ...
    Private Sub dgv_servicios_CellEndEdit(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgv_servicios.CellEndEdit
        ...
        If e.RowIndex > -1 AndAlso e.ColumnIndex > -1 Then
            ...
            Dim Actualiza As String = "UPDATE ContratosDetalle SET dcon_activo='" + Me.dgv_servicios.Rows(e.RowIndex).Cells("Dgv_Estado").Value.ToString() + "' ," +
                                      "Dcon_Valor='" + Me.dgv_servicios.Rows(e.RowIndex).Cells("Valor").Value.ToString() + "', " +
                                      "Dcon_Moneda='" + Me.dgv_servicios.Rows(e.RowIndex).Cells("Dgv_Moneda").Value.ToString() + "', " +
                                      "Dcon_Medida='" + Me.dgv_servicios.Rows(e.RowIndex).Cells("Dgv_Medida").Value.ToString() + "'  " +
                                      "WHERE Dcon_Codigo='" + contrato.ToString() + "' AND Dcon_ServCod='" + Me.dgv_servicios.Rows(e.RowIndex).Cells("Codigo").Value.ToString() + "'"
    ...
    Private Sub BtnAsignar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnAsignar.Click
        If MsgBox("Esta Seguro de actualizar toda la información", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "Aviso") = vbYes Then
            Dim sql As String = "UPDATE ContratosDetalle SET dcon_moneda='" + CbMonedas.SelectedValue.ToString() + "' WHERE dcon_codigo='" + contrato + "'"
